...
import { Menu, CheckCircle2, ClipboardList, LogOut, X, Building2, TrendingUp, CreditCard, Home as HomeIcon } from "lucide-react";
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "52px 28px 80px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {CONNECTION_TYPES.map((c) => (
            ...
            >
              <div style={{ width: 44, height: 44, borderRadius: "50%", background: "rgba(92,122,101,0.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {c.icon}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontFamily: serif, fontSize: 16, color: ink, margin: "0 0 3px", fontWeight: 400 }}>{c.label}</p>
                <p style={{ fontSize: 13, color: muted, margin: 0, lineHeight: 1.5 }}>{c.description}</p>
              </div>
              <div style={{ flexShrink: 0, background: "rgba(92,122,101,0.08)", border: `1px solid rgba(92,122,101,0.2)`, borderRadius: 8, padding: "7px 14px", fontSize: 12, color: muted, fontFamily: sans, fontWeight: 500, whiteSpace: "nowrap" }}>
                Coming soon
              </div>
            </div>
          ))}
        ...
        <p style={{ fontSize: 13, color: muted, textAlign: "center", marginTop: 36, lineHeight: 1.6 }}>
          Account connections will use bank-grade encryption and read-only access. Your credentials are never stored.
        </p>
      </div>
    </div>
  );
