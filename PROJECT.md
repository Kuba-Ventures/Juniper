...
*Last updated: 2026-08-07 19:30 ET by kuba-vault*
...
Juniper helps engaged and newly married couples model the financial tradeoffs behind big decisions (buying a home, combining finances, paying down debt, planning for a baby, prenups) and align on them before commitments are made. All five domains run end-to-end in production at `juniper-api-server.vercel.app`; the phase is post-MVP GTM iteration. The dark-mode toggle is now **fixed, merged, and live in production**. PR #116 (merged) had put the "Dark mode" switch in `ProfileSettings` + the old `app-shell.tsx`, but the live app was already rebuilt on the newer **JuniperApp** shell (`pages/juniper-app.tsx` + `components/juniper/*`) — so the control shipped into dead code and was never reachable. The theme *engine* was live the whole time (`src/lib/theme.tsx` `ThemeProvider`/`useTheme`, `localStorage["juniper_theme"]`, OS default, toggles `.dark` on `<html>`, pre-paint script in `index.html`, dark palette in `.dark .jnpr` in `src/styles/juniper.css`) — which is why the app honored dark mode but offered no way to change it. PR #117 (`claude/dark-mode-settings-modal-toggle`, merge commit `7de97c1`, merged 2026-08-07) moved the toggle into the **live** `components/juniper/settings-modal.tsx` (Account → Appearance), wired to `useTheme()` and styled with `.jnpr` tokens, and reverted PR #116's edits to the retired shell files so there's one source of truth. Verified live in the served production bundle at `juniper-api-server.vercel.app`. Because the live JuniperApp shell themes through `.dark .jnpr` in the design system, the whole shell already follows dark mode — the old "only core surfaces are themed" caveat was about the retired shell and no longer describes the live app. The last merged product work (`main`, PRs #28–#33) remains the GTM push — WAU instrumentation, marketplace, portfolio summary, and Plaid account linking on Sandbox. Two compliance gates still block real monetization: affiliate URLs are `example.com` placeholders pending licensing, and Plaid real-data access needs Plaid's production review.
...
- **Next milestone:** Dark-mode fix merged + live (PR #117, merge commit `7de97c1`). Longer-term gate: approved affiliate programs and Plaid Production access before monetized surfaces and real bank data go live — TBD
- **Flags:** shipping (dark-mode toggle merged to `main` via PR #117 and deployed to production)

---

## Where we are right now

**Dark-mode toggle fixed, merged, and live in production.** PR #117 (`claude/dark-mode-settings-modal-toggle`, merge commit `7de97c1`, merged 2026-08-07) is on `main` and deployed — the toggle in `components/juniper/settings-modal.tsx` (Account → Appearance → Dark mode) is confirmed in the served production bundle at `juniper-api-server.vercel.app`. The bug it fixed: PR #116 (merged) had added the "Dark mode" switch to `ProfileSettings` and the old `app-shell.tsx`, but the live app was already rebuilt on the newer **JuniperApp** shell (`pages/juniper-app.tsx` + `components/juniper/*`), which those files no longer feed. So the toggle shipped into dead code — never reachable in the running app (the settings dialog the live app actually opens is `components/juniper/settings-modal.tsx`, rendered from `components/juniper/app-frame.tsx`). The theme *engine* had always been live, which is why the app honored dark mode but gave the user no control: `src/lib/theme.tsx` (`ThemeProvider` + `useTheme`, persists to `localStorage["juniper_theme"]`, OS default, toggles `.dark` on `<html>`), the pre-paint script in `index.html`, `App.tsx` wrapping in `ThemeProvider`, and the dark palette under `.dark .jnpr` in `src/styles/juniper.css` (this is what themes the live design system — not the `.dark {}` block in `index.css`, which serves the retired shadcn surfaces). PR #117 (1) added the "Dark mode" toggle to the live `settings-modal.tsx`, wired to `useTheme()`, styled with `.jnpr` tokens, applies immediately; and (2) reverted PR #116's edits to the retired files (`app-shell.tsx`, `app-sidebar.tsx`, `pages/dashboard.tsx`, `domain-tile.tsx`, `portfolio-summary.tsx`, `profile-settings.tsx`) back to their pre-#116 state, so there's a single source of truth and no divergent second settings modal. Because the live JuniperApp shell themes via `.dark .jnpr` in the design system, the whole shell already follows dark mode — the earlier "only core surfaces themed" caveat was about the retired shell and is largely moot for the live app. Prior GTM work (WAU, marketplace, portfolio summary, Plaid Sandbox linking) is merged to `main`; the two compliance gates (affiliate licensing, Plaid Production) are unchanged.
