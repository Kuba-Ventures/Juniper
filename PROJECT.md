...
*Last updated: 2026-08-07 17:45 ET by kuba-vault*
...
- **Next milestone:** Merge the dark-mode fix (PR pending on `claude/dark-mode-settings-modal-toggle`), putting the toggle in the live settings modal. Longer-term gate: approved affiliate programs and Plaid Production access before monetized surfaces and real bank data go live — TBD
- **Flags:** shipping (dark-mode fix in progress on `claude/dark-mode-settings-modal-toggle`, commit `807fd98`, PR pending; PR #116 already merged to `main`)
...
**Fixing a broken dark-mode toggle** on branch `claude/dark-mode-settings-modal-toggle` (commit `807fd98`, PR pending — not merged). The bug: PR #116 (merged) had added the "Dark mode" switch to `ProfileSettings` and the old `app-shell.tsx`, but the live app was already rebuilt on the newer **JuniperApp** shell (`pages/juniper-app.tsx` + `components/juniper/*`), which those files no longer feed. So the toggle shipped into dead code — it was never reachable in the running app (the settings dialog the live app actually opens is `components/juniper/settings-modal.tsx`, rendered from `components/juniper/app-frame.tsx`). The theme *engine* had always been live, which is why the app honored dark mode but gave the user no control: `src/lib/theme.tsx` (`ThemeProvider` + `useTheme`, persists to `localStorage["juniper_theme"]`, OS default, toggles `.dark` on `<html>`), the pre-paint script in `index.html`, `App.tsx` wrapping in `ThemeProvider`, and the dark palette under `.dark .jnpr` in `src/styles/juniper.css` (this is what themes the live design system — not the `.dark {}` block in `index.css`, which serves the retired shadcn surfaces). This change (1) adds the "Dark mode" toggle to the live `settings-modal.tsx` (Account settings → Appearance), wired to `useTheme()`, styled with `.jnpr` tokens, applies immediately; and (2) reverts PR #116's edits to the retired files (`app-shell.tsx`, `app-sidebar.tsx`, `pages/dashboard.tsx`, `domain-tile.tsx`, `portfolio-summary.tsx`, `profile-settings.tsx`) back to their pre-#116 state, so there's a single source of truth and no divergent second settings modal. Because the live JuniperApp shell themes via `.dark .jnpr` in the design system, the whole shell already follows dark mode — the earlier "only core surfaces themed" caveat was about the retired shell and is largely moot for the live app. typecheck 0 errors, build green. Prior GTM work (WAU, marketplace, portfolio summary, Plaid Sandbox linking) is merged to `main`; the two compliance gates (affiliate licensing, Plaid Production) are unchanged.
...
- [ ] Merge the dark-mode fix (PR pending on `claude/dark-mode-settings-modal-toggle`, commit `807fd98`) — moves the toggle into the live settings modal and reverts #116's dead-shell edits — Finley
