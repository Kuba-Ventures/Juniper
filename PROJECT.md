...
*Last updated: 2026-08-07 by kuba-vault*
...
Juniper helps engaged and newly married couples model the financial tradeoffs behind big decisions (buying a home, combining finances, paying down debt, planning for a baby, prenups) and align on them before commitments are made. All five domains run end-to-end in production at `juniper-api-server.vercel.app`; the phase is post-MVP GTM iteration. This session builds a **dark-mode toggle** for the web app on branch `feat/dark-mode-toggle` (PR pending, not yet merged): a new `ThemeProvider`/`useTheme` system (`src/lib/theme.tsx`) that persists to `localStorage`, defaults to the OS preference, and toggles the `dark` class on `<html>`, plus a completed warm "juniper-dark" palette that replaces the `red` placeholder tokens that had left the scaffolded `.dark {}` block non-functional. A "Dark mode" switch lives in Profile → Account → Appearance. Scope is deliberately partial: only the core shell (app shell, sidebar, settings modal) and the dashboard follow the theme; Chat, Saved plans, Connections, and the marketing/auth pages still use fixed light hex and stay light until a follow-up migration. The last merged work (`main`, PRs #28–#33) remains the GTM push — WAU instrumentation, marketplace, portfolio summary, and Plaid account linking on Sandbox. Two compliance gates still block real monetization: affiliate URLs are `example.com` placeholders pending licensing, and Plaid real-data access needs Plaid's production review.
...
- **Next milestone:** Merge the dark-mode toggle (PR on `feat/dark-mode-toggle`, pending review), then a follow-up pass to theme the remaining surfaces (Chat, Saved plans, Connections, marketing/auth). Longer-term gate: approved affiliate programs and Plaid Production access before monetized surfaces and real bank data go live — TBD
- **Flags:** shipping (dark-mode toggle in progress on `feat/dark-mode-toggle`, PR pending; last merge `main` at PR #33/#34)
...
Building a **dark-mode toggle** on branch `feat/dark-mode-toggle` (committed, PR pending review — not merged). Dark mode had been scaffolded but never finished: the `.dark {}` block in `src/index.css` held `red` placeholders for every token, so it never actually rendered. This change fills in a complete warm "juniper-dark" palette — a deep green-charcoal background (`156 16% 10%` / `#15201b`), warm off-white text, and the brand juniper-green accent brightened for the dark ground. New theme system in `src/lib/theme.tsx`: a `ThemeProvider` + `useTheme` hook that persists the choice to `localStorage["juniper_theme"]`, defaults to the OS `prefers-color-scheme` on first visit, and toggles the `dark` class on `<html>`. A tiny pre-paint script in `index.html` (mirrors `readInitialTheme()`) applies the saved theme before React mounts to avoid a flash. The control is a "Dark mode" switch in Profile → Account → Appearance (`profile-settings.tsx`); it applies immediately, not gated on Save. Surfaces converted off hardcoded light hex onto the existing CSS variables so they follow the theme: the app shell, sidebar, settings modal, and the dashboard (domain tiles + portfolio summary). **Scope is intentionally partial** — Chat, Saved plans, and Connections, plus the marketing/landing and auth pages, still use fixed light hex and stay light until a follow-up pass migrates them onto the variables. That follow-up is the tracked doc debt. Prior GTM work (WAU, marketplace, portfolio summary, Plaid Sandbox linking) is merged to `main`; the two compliance gates (affiliate licensing, Plaid Production) are unchanged.
...
**Frontend / UI**
- **Dark-mode toggle** *(in progress — branch `feat/dark-mode-toggle`, PR pending, not merged to `main`)*: theme system in `src/lib/theme.tsx` (`ThemeProvider` wraps the app in `App.tsx`; `useTheme` hook) persists to `localStorage["juniper_theme"]`, defaults to the OS `prefers-color-scheme`, and toggles the `dark` class on `<html>`. A pre-paint script in `index.html` applies the saved theme before React mounts (no flash). Palette lives in `src/index.css` — the previously-`red`-placeholder `.dark {}` block now holds a complete warm "juniper-dark" set (bg `156 16% 10%`, warm off-white fg, brightened juniper-green primary/accent). Control: a "Dark mode" `Switch` in Profile → Account → Appearance (`components/profile/profile-settings.tsx`), applies immediately (not gated on Save). **Themed surfaces:** app shell, sidebar, settings modal, dashboard (domain tiles + portfolio summary), all converted from hardcoded light hex to `hsl(var(--…))`. **Not yet themed** (still fixed light hex): Chat, Saved plans, Connections, marketing/landing, auth pages
...
- **2026-08-07 — Dark mode ships core surfaces only, rest in a follow-up** — Rather than block on converting every page off hardcoded hex, the toggle themes the app shell, sidebar, settings modal, and dashboard now; Chat, Saved plans, Connections, and the marketing/auth pages stay light until a follow-up migrates them onto the CSS variables. Ships a working, coherent dark experience on the surfaces users spend time in without a giant one-shot diff. Tracked as an open loop.
...
- [ ] Merge the dark-mode toggle (PR on `feat/dark-mode-toggle`, pending review) — Finley
- [ ] Dark-mode follow-up: migrate the remaining surfaces off fixed light hex onto the CSS variables — Chat, Saved plans, Connections, and the marketing/landing + auth pages — so they follow the theme too — Finley
...
- **Dark mode themes only part of the app** — with dark mode on, the app shell, sidebar, settings modal, and dashboard follow the theme, but Chat, Saved plans, Connections, and the marketing/auth pages still render in fixed light hex. Navigating to those pages in dark mode gives a jarring light-on-dark seam until the follow-up migration lands. Known and tracked, not a regression.
